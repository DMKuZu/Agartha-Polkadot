...
// POST /api/deals/[id]/accept — Arbiter or freelancer accepts the deal
export async function POST(
  ...
  if (deal.status === 'cancelled') {
    return NextResponse.json({ error: 'Deal has been cancelled' }, { status: 409 });
  }
  if (deal.status === 'accepted' || deal.status === 'deployed') {
    return NextResponse.json({ error: 'Deal is already accepted or deployed' }, { status: 409 });
  }
  ...
  if (isArbiter)    updates.arbiter_accepted    = true;
  if (isFreelancer) updates.freelancer_accepted = true;

  // Determine if both will have accepted after this update
  const bothAccepted =
    (isArbiter    ? true : !!deal.arbiter_accepted) &&
    (isFreelancer ? true : !!deal.freelancer_accepted);
  ...
  if (error) {
    await logError('/api/deals/[id]/accept', error);
